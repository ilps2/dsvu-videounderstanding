...
def _classify_scenes_functional(video_path, dur, scenes_path):
    """函数式场景分类（无 MV）：帧差检测边界 + 运动分类。
    dsvu 替代：原 SceneClassifier 类已随 scene_classifier 函数式重构移除（遗留 bug），
    无 MV 时不再用 uniform 权重，改用帧级真实场景信息。
    返回 (scene_w, vbound, ss, n_boundaries)。"""
    import numpy as np
    try:
        sys.path.insert(0, str(BASE))
        from scene_classifier import detect_scene_boundaries, classify_scenes_by_motion
        bounds = detect_scene_boundaries(video_path)
        labels = classify_scenes_by_motion(video_path)
        vbound = set(int(b) for b in bounds if 0 <= int(b) <= int(dur))
        sec_labels = [labels.get(s, "unknown") for s in range(int(dur) + 1)]
        import pandas as pd
        ss = pd.DataFrame({"scene": sec_labels}, index=range(int(dur) + 1))
        ss.index.name = "sec"
        ss.to_csv(scenes_path, header=["scene"])
        scene_w = np.ones(int(dur) + 1) * 0.8
        for b in vbound:   # 边界秒权重下调，作为注意力弱信号（对齐原 MV 语义）
            scene_w[b] = 0.5
        n_boundaries = len(vbound)
        print(f"  Scenes(functional): {dict(ss['scene'].value_counts())} | {n_boundaries} boundaries")
        return scene_w, vbound, ss, n_boundaries
    except Exception as e:
        print(f"  ⚠ Functional scene classification failed ({e}) → uniform weights")
        scene_w = np.ones(int(dur) + 1) * 0.8
        return scene_w, set(), None, 0
...
def encode_video(video_path: Path, output_dir: Path = None,
    ...
    if skip_mv:
        # dsvu：--skip-mv 语义为彻底跳过（不提取、不复用），场景分类走函数式降级。
        # 原因：SceneClassifier 类已随 scene_classifier 函数式重构移除，mv.npz 存在会
        # 触发不存在的 import 而崩（原项目遗留 bug）。
        print("  Skipping MV extraction (--skip-mv) → functional scene classification")
    else:
    ...
    if has_mv:
        try:
            ...
            # dsvu：SceneClassifier 类已随函数式重构移除，失败时降级到函数式场景分类
            print(f"  ⚠ MV scene classifier unavailable ({e}) → functional fallback")
            has_mv = False
            scene_w, vbound, ss, n_boundaries = _classify_scenes_functional(
                str(video), dur, scenes_path)
    else:
        # 无 MV：用函数式场景分类（基于帧差/运动），避免 uniform 损失场景信息
        scene_w, vbound, ss, n_boundaries = _classify_scenes_functional(
            str(video), dur, scenes_path)
...
def clip_from_avis(avis_dir: Path, max_clips: int = 3,
    ...
    if has_mv:
        try:
            ...
            # dsvu：SceneClassifier 已移除（遗留 bug）→ 空边界降级
            print(f"  ⚠ MV boundary skip ({e}) → empty vbound")
